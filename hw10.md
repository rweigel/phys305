...
1\. At this point (and actually any point on the wire), $\cos\theta=d/r$ and $\sin\theta=b/r$, where $r=\sqrt{b^2+d^2}$

$$\mathbf{B}_{ext}=\frac{\mu_o}{4\pi}\frac{m_o}{(b^2+d^2)^{5/2}}\left(2d\hat{\mathbf{r}}+b\hat{\boldsymbol{\theta}}\right)$$


2\. From the diagram, at any point on the wire, the field has components in $+z$ and $+s$ directions (for small enough $\theta$). The cross product of $B_s$ with $\mathbf{I}$ gives a $+\zhat$ force.
...
(Earlier I noted that for small enough $\theta$, $\mathbf{B}_{ext}$ has a $+z$ component. From this equation, we can see that the requirement for this is $3\cos^2\theta-1 > 1$.)
...
Normally we would convert $\hat{\mathbf{s}}$ to cartesian unit vectors before integration. However, we know that the integral of $d\phi\hat{\mathbf{s}}$ from $0$ to $2\pi$ is zero from a diagram (see discussion in [integrating unit vectors](vectors.html#integrating); we also expect this physically -- if we pick two points on opposite sides of the wire, the force on them in the $\hat{\mathbf{s}}$ direction is equal and opposite. As a result, we could drop the $\hat{\mathbf{s}}$ term at this point. In addition, the $z$--component is constant on the loop because $\theta$ is constant. So integration is not needed -- one can use $F=ILB$ with $L=2\pi b$. In the following, details on the math that leads to these conclusions is shown.
...
$\mathbf{F}=6\pi IB_ob \cos\theta\sin\theta\zhat$
...
Bottom: $B_z=\frac{\mu_o I}{4\pi (b/2)}\frac{-2a}{\sqrt{a^2+b^2}}=-\frac{\mu_o I}{\pi b}\frac{a}{\sqrt{a^2+b^2}}$ 

The top wire contributes the same field as the bottom, so.

Top: $B_z=-\frac{\mu_o I}{\pi b}\frac{a}{\sqrt{a^2+b^2}}$ 

The field for the left and right wire can be found by swapping $a$ and $b$ in the above equation.

Left: $B_z=-\frac{\mu_o I}{\pi a}\frac{b}{\sqrt{a^2+b^2}}$ 

Right: $B_z=-\frac{\mu_o I}{\pi a}\frac{b}{\sqrt{a^2+b^2}}$ 
...
Within these limits of integration, $\cos u$ is positive, so we can drop the absolute value and integrate.
...
2\. For an infinite wire, $B=\mu_o I/2\pi s$, where $s$ is the perpendicular distance from the wire. The top and bottom wire will both create a field with this magnitude in the $-z$ direction (from the right-hand rule). For both wires, the perpendicular distance is $b/2$. Therefore, $\mathbf{B}=2\mu_o I/\pi b\zhat$.

The field from the top and bottom wires from part 1. is

$B_z=-\frac{2\mu_o I}{\pi b}\frac{a}{\sqrt{a^2+b^2}}$

Factoring out $a$ gives

$B_z=-\frac{2\mu_o I}{\pi b}\frac{1}{\sqrt{1+b^2/a^2}}$

For $a\gg b$,

$B_z\simeq -\frac{2\mu_o I}{\pi b}$

which is the same result fround using the equation for the field due to infinite wire for the bottom and top wires.
...
If the origin is the center of the circle, $x$ is up, and $y$ is to the right, then $\mathbf{I}=I\hat{\boldsymbol{\phi}}$, $dl=Rd\phi$, $\mathbf{r}=\mathbf{0}$, and $\mathbf{r}'=R\hat{\mathbf{s}}$.

$\mathbf{B}=\frac{\mu_o}{4\pi}\int_{-\pi/2}^{\pi}Rd\phi \frac{\hat{\boldsymbol{\phi}}\times (-R\hat{\mathbf{s}})}{R^3}=\zhat\frac{\mu_o}{4\pi}\frac{1}{R}\int_{-\pi/2}^{\pi}d\phi=\frac{\mu_oI}{4R}\zhat$

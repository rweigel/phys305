...
\label{section:computing_electric_flux}

In the previous section, the charge distribution was such that we could find a Gaussian surface for which Equation~\ref{eqn:simple2} applied. In this section, we will consider the case where the electric field is at an angle to a surface. The types of calculations described in this section will not actually be used to find an electric field due to a continuous charge distribution. However, the calculations are important to understand and will be used in the future. In addition to the resources posted on the course web page, the two supplemental videos \href{https://www.youtube.com/watch?v=9MN0eTC-vzQ}[1] and \href{https://www.youtube.com/watch?v=yOv4xxopQFQ}[2] describe most of what is covered in this section.
...
\fbox{
    ...
    \begin{enumerate}
        \item $E$ is constant on a flat surface of area $A$, and
        \item the angle of \mathbf{E} with respect to $A$ is constant.
    \end{enumerate}
    }
}
...
\clearpage
\end{comment}
\begin{tcolorbox}[enhanced,breakable,parbox=false,colframe=blue!50!blue,colback=white,height fill,title=Example 4.1 - Flux through faces of a cube]
...
\begin{tcolorbox}[enhanced,breakable,parbox=false,colframe=blue!50!blue,colback=white,height fill,title=Example 4.2 - Flux through faces of a cube]
...
\begin{tcolorbox}[enhanced,breakable,parbox=false,colframe=black!50!black,colback=white,height fill,title=Problem 4.1 - Flux through faces of a cube]

\input{Gauss/figures/Flux_Through_Cube_3}

Find the flux through the faces of the cube with side length $a$ when the electric field is everywhere in the $+y$ direction.
